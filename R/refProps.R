...
#'
#' @examples
#' refProps(1000) # Generates a data frame with 1000 entries based on the defined distributions.
#' @export
refProps <- function(n = 1000) {
  set.seed(123)
  ...
  data <- data.frame(
    hair_colour = factor(hair_colour, labels = c("Black", "Brown", "Blonde", "Red", "Gray")),
    skin_colour = factor(skin_colour, labels = c("Very light", "Light", "Medium", "Dark", "Very dark")),
    eye_colour = factor(eye_colour, labels = c("Brown", "Blue", "Green", "Gray", "Hazel"))
  )
